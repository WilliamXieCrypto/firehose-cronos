...
ADD /firecronos /app/firecronos

COPY tools/firecronos/motd_generic /etc/
COPY tools/firecronos/motd_node_manager /etc/
COPY tools/firecronos/99-firehose.sh /etc/profile.d/
COPY tools/firecronos/scripts/* /usr/local/bin

ENTRYPOINT ["/app/firecronos"]
